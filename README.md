...
- **QR 코드 정상화**
  - 장식용 SVG(스캔 불가, 위치마커 좌표 버그로 도형 겹침)를 `qrcode` 라이브러리 기반 실제 스캔 가능한 QR로 교체
  - 인코딩→디코딩 라운드트립 테스트로 스캔 정확성 검증
  - QR/URL이 대시보드 자신이 아니라 실제 스탬프 적립 화면인 고객용 앱 배포 도메인(`https://rebot-app-customer-facing-page.vercel.app/:storeCode`)을 가리키도록 수정
- **QR 카드 레이아웃 수정**
  - 좁은 그리드 컬럼 안에서 가로 배치로 인해 텍스트·URL 입력창·다운로드 버튼이 카드 밖으로 넘치던 문제 수정 (세로 스택 + `min-w-0`)
- **개발 환경 정리**
  - `.gitattributes` 추가로 CRLF/LF 줄바꿈 뒤섞임 재발 방지
  - WSL/Windows 혼합 환경에서 `npm run dev` 시 리눅스 네이티브 모듈 누락 문제 확인 및 `npm install` 재실행으로 해결 (코드 변경 없음)
