...
### 1-2. 2026-07-03 톤 리비전 (2차) — 감정 호소 제거 + 단계별 예시 문구 확정

PR #3 재리뷰 중 사용자가 4단계(safe/watch/danger/churned) 각각의 as-is/to-be 예시를 직접 제시하며 톤을 다시 조정했다. **1-1에서 한 번 더 달라진 점**:

- **감정 호소 표현 전면 삭제**: "서운하다", "그리움", "걱정 반 그리움 반" 등 danger/churned 단계의 감성적 표현을 모두 제거하고, 혜택과 재방문 명분을 명확히 제시하는 담백한 톤으로 교체. (1-1에서는 "danger는 서운함·그리움을 담아"라고 반대로 지시했었음 — 이번에 뒤집힘.)
- **통일된 인사 형식**: 4단계 모두 `"{이름 또는 없음} 고객님, 안녕하세요.\n{storeName}입니다."`로 시작 (기존엔 단계별로 인사말이 제각각이었음: "늘 감사드립니다", "오랜만에 인사 올립니다" 등).
- **매장명 하드코딩 버그 수정**: 폴백 템플�릿이 `"리봇 베이커리"`를 리터럴 문자열로 하드코딩해 실제 매장명(`storeName`, 예: "리봇 카페 & 베이커리")과 불일치하는 문제 발견 → 전부 `${storeName}` 변수로 교체.
- **safe(일반) 단계 전용 문구 신설**: 기존엔 `getFallbackMessage()`에 `danger`/`watch`/그 외(else) 3개 분기만 있어 safe와 churned가 같은 문구를 공유했음 → safe 전용 분기 추가.
- 프롬프트(`buildMessagePrompt`)의 톤 지시문과 폴백 템플릿(`getFallbackMessage`) 양쪽에 동일하게 반영.

이 리비전이 최종본이며, 실제 코드(`src/lib/prompts.ts`, `src/lib/ai-server.ts`)에 반영·커밋됨.
